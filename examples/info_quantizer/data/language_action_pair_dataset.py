...
def collate(
    ...
    if samples[0].get("action", None) is not None:
        ...
        action = action.index_select(0, sort_order)
...
class LanguageActionPairDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        if act is not None:
            assert len(src) == len(
                act
            ), "Source and Action must contain the same number of examples"
        self.src = src
        self.tgt = tgt
        self.act = act
        self.src_sizes = np.array(src_sizes)
        self.tgt_sizes = np.array(tgt_sizes) if tgt_sizes is not None else None
        self.act_sizes = np.array(act_sizes) if act_sizes is not None else None
        self.sizes = (
        ...
        if num_buckets > 0:
            ...
            if self.act is not None:
                self.act = BucketPadLengthDataset(
                    self.act,
                    sizes=self.act_sizes,
                    num_buckets=num_buckets,
                    pad_idx=self.act_dict.pad(),
                    left_pad=self.left_pad_target,
                )
                self.act_sizes = self.act.sizes
                logger.info(
                    "bucketing target lengths: {}".format(list(self.tgt.buckets))
                )
    ...
    def __getitem__(self, index):
        ...
        act_item = self.act[index] if self.act is not None else None
