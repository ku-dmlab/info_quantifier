...
def load_mt_model(model_path, data_bin=None, return_dict=False):
...
def load_policy_model(args, nmt=False):
    utils.import_user_module(args)
    filename = args.policy_model_path
...
class BeamDecoding():
    def __init__(
        ...
        self.min_len = 2
        self.max_len = 200
        # self.max_len = self.model.max_decoder_positions()

        self.bos_token = self.eos
    ...
    def greedy_decode(
        ...
        assert prev_tokens is None or prev_tokens.size(0) == 1
        eos_tensor = torch.tensor([[self.eos]]).to(self.device)
        if prev_tokens is None or prev_tokens.size(-1) == 0:
            prev_tokens = eos_tensor
        
        if prev_tokens[0, 0] != self.eos:
            prev_tokens = torch.cat(
                (eos_tensor, prev_tokens), -1
            )

        # incremental = torch.jit.annotate(
        ...
        while True:
            ...
            lprobs = self.model.single_model.get_normalized_probs(
                [out[0][:, -1:]], log_probs=True
            )
            token = lprobs.argmax(-1)
            prev_tokens = torch.cat((prev_tokens, token), -1)
            if token == self.eos:
                break

        return prev_tokens[:, 1:]
    ...
    def one_step_decode(
        ...
        token = lprobs.argmax(-1)[0, 0].item()
    ...
    def decode(
        ...
        inner_states: Optional[Tensor] = None
        ...
        for step in range(max_len + 1):
            ...
            if inner_states is None:
                inner_states = torch.empty(
                    bsz * beam_size, max_len + 1, inners.size(-1)
                ).to(scores)
            # inners (1, bbsz, dim) -> bbsz, step, dim
            inner_states[:, step:step+1] = inners.permute(1,0,2).contiguous()
            ...
            if len(finalized_sents) > 0:
                ...
                if inner_states is not None:
                    inner_states = inner_states.view(bsz, -1)[batch_idxs].view(
                        new_bsz * beam_size, inner_states.size(1), -1
                    )
            ...
            if inner_states is not None:
                attn[:, : step + 1] = torch.index_select(
                    attn[:, : step + 1], dim=0, index=active_bbsz_idx
                )
        ...
        # out_size = out_active_mask.size(0)
        # _finalized = torch.jit.annotate(
        #     List[List[Dict[str, Tensor]]],
        #     [torch.jit.annotate(List[Dict[str, Tensor]], []) for _ in range(out_size)],
        # )

        # out_idxs = torch.arange(out_size)[out_active_mask.cpu()]
        # for oi, final in zip(out_idxs, finalized):
        #     i = oi.item()
        #     _finalized[i] = final
    ...
    def finalize_hypos(
        ...
        inner_clone = (
            inner_states.index_select(0, bbsz_idx)[:, : step + 1]
            if inner_states is not None
            else None
        )
        ...
        for i in range(bbsz_idx.size()[0]):
            ...
            if len(finalized[sent_list[i]]) < beam_size:
                ...
                hypo_inner = torch.empty(0) 

                if attn_clone is not None:
                    # remove padding tokens from attn scores
                    hypo_attn = attn_clone[i]

                if inner_clone is not None:
                    hypo_inner = inner_clone[i]
