...
class SubsampleDataset(BaseWrapperDataset):
    ...
    def ordered_indices(self):
        ...
        if len(self.sizes.shape) > 1:
            sizes = self.sizes.transpose()[0]
            sizes_trg = self.sizes.transpose()[1]
        else:
            sizes = self.sizes
        filter_mask = (( sizes > 5 ) & ( sizes < 200 )) & (( sizes_trg > 5 ) & ( sizes_trg < 200 ))
        self.indices = self.indices[filter_mask]
        sizes = sizes[filter_mask]
        order = [order[0][filter_mask]]
        order.append(sizes)
        #
